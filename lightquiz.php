...
/**
 * Internal library of functions for module Lightquiz
 *
 * All the lightquiz specific functions, needed to implement the module
...
//require oauthlib for lightquiz
...
class lightquiz {
  ...
  public $host = "http://www.lightquiz.com/platform/";
  /* Set timeout default. */
  public $timeout = 30;
  /* Set connect timeout. */
  ...
  public $http_info;
  /* Set the useragnet. */
  public $useragent = 'lightquiz oauth v1.0';
   ...
   * Set API URLS
   */
  function accessTokenURL()  {  return $this->host . 'accessToken'; }
  //function accessTokenURL() { return $this->host . 'authenticate'; }
  function authenticateURL() { return $this->host . 'authenticate'; }
  function authorizeURL()    { return $this->host . 'authorize'; }
  function requestTokenURL() { return $this->host . 'requestToken'; }
  ...
  function parseReturn($xmlstring, $type){
		$ret = array();
		$parsed_element =  simplexml_load_string($xmlstring);
		...
		if($parsed_element){
			switch($type){
				case 'request':
					...
					break;
				case 'access':
					$token = $parsed_element->accessToken->token;
					$token_secret = $parsed_element->accessToken->token_secret;
					break;
				case 'authenticate':
					$token = $parsed_element->authenticate->token;
					$token_secret = $parsed_element->authenticate->token_secret;
					break;
			}
  ...
  function getAuthorizeURL($token,$ec_user) {
       return $this->authenticateURL() . "?user={$ec_user}&oauth_token={$token}";
  }
